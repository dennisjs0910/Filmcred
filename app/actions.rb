...
post '/users/:id/request' do
  req = Request.new
  req.company_id = current_user.id
...
post '/users/:id/approve' do
  req = Request.where(worker_id: params[:id]).last
  req.pending = false
  req.save
  redirect "/users/#{current_user.id}"
end

get '/company_documents' do
  @documents = current_user.requests.last.worker.documents.last
  erb :'company_documents/index'
end
